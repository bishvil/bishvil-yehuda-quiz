...
async function callBegin(pin: string, questionId: string) {
  const { POST } = await import(
    "@/app/api/host/[pin]/question/begin-answering/route"
  );
  const response = await POST(
    new Request(`http://localhost:3000/api/host/${pin}/question/begin-answering`, {
      method: "POST",
...
describe("POST /api/host/[pin]/question/begin-answering", () => {
  it("transitions a presenting sync video question to answering", async () => {
    ...
    const result = await callBegin(fixtures.pin, fixtures.questionId);

    expect(result.status).toBe(200);
    expect(result.body).toMatchObject({ status: "answering" });
    ...
    `;
    expect(row).toMatchObject({ status: "answering" });
