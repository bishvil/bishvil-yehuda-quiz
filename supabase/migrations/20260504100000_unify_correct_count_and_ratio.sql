-- Unify scoring contract — ADR-0006 update 2026-05-04.
--
-- Two behaviour changes vs the previous submit_answer (in
-- 20260504010000_drop_legacy_map_path.sql):
--
-- 1. correctness_ratio is no longer NULL for binary types (single /
--    truefalse / image). The RPC writes 1.0 on a correct answer and 0.0
--    on a wrong one. UI consumers stop forking on a NULL check.
--
-- 2. participant_scores.correct_count and session_participants.streak now
--    gate on v_score > 0, not v_is_correct. This unifies the two partial
--    types — multi (which only flagged is_correct on ratio == 1) is now
--    counted on every partial-credit answer that earned points, matching
--    geo's behaviour.
--
-- The function return shape is unchanged, so callers do not need to
-- update. We DROP and re-CREATE because Postgres rejects CREATE OR
-- REPLACE when the function body changes meaningfully and we want a
-- clean replacement (no stale plan caches).
--
...
begin
  ...
  -- Permissive correct_count: any answer that earned points contributes 1.
  -- Multi-partial answers (Jaccard 0 < r < 1) now bump the counter the
  -- same way geo-partial answers (proximity 0 < r < 1) already did.
  ...
  -- Streak follows the same permissive rule: any scoring answer extends
  -- the streak; a zero-score answer breaks it.
